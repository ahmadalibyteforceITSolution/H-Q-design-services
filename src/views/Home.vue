<template>
  <div class="space-y-16 pb-16">
    ...
    <!-- TRENDING ARCHITECTURAL SEARCH DIRECTORY SHORTCUT -->
    <section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
      <div class="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 border border-slate-800 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="space-y-2 max-w-3xl">
          <div class="flex items-center gap-2">
            <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase bg-[#088C7E] text-white tracking-wider flex items-center gap-1.5">
              ...
              <span>SEO Keyword Hub</span>
            </span>
            <span class="text-xs font-bold text-slate-400">1,008+ Indexed Search Topics</span>
          </div>
          <h3 class="text-xl sm:text-2xl font-black text-white">
            Looking for specific floor plans, elevations, or society bylaws?
          </h3>
          <p class="text-xs sm:text-sm text-slate-400">
            Explore our complete Trending Searches Directory covering 3 Marla to 4 Kanal villa blueprints, modern front elevation 3D renders, luxury interior designs, and construction cost estimates across Pakistan.
          </p>
        </div>
        <router-link 
          to="/keywords-directory"
          class="px-6 py-3.5 rounded-2xl bg-[#088C7E] hover:bg-[#066D62] text-white font-black text-xs uppercase tracking-wider transition-all hover:scale-105 shadow-xl flex items-center gap-2 whitespace-nowrap cursor-pointer"
        >
          <i class="fa-solid fa-folder-open"></i>
          <span>Open Keyword Directory →</span>
        </router-link>
      </div>
    </section>
...
const goToProperties = () => {
  if (activeSearchTab.value === 'NEW PROJECTS') {
    router.push('/projects')
    return
  }

  let purpose = 'All'
  if (activeSearchTab.value === 'BUY') purpose = 'For Sale'
  if (activeSearchTab.value === 'RENT') purpose = 'For Rent'
  if (activeSearchTab.value === 'PLOTS') purpose = 'Plots'
  if (activeSearchTab.value === 'COMMERCIAL') purpose = 'Commercial'
  ...
  if (searchPlot.value !== 'All') query.size = searchPlot.value
